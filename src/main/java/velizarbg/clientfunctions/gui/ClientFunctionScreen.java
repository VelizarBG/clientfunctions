...
public class ClientFunctionScreen extends Screen {
	...
	protected void init() {
		...
		runButton = addDrawableChild(
			new ButtonWidget(
				relWidth.getAsInt() + BOX_WIDTH - 75 + 1, relHeight.getAsInt() + BOX_HEIGHT + 1, 75, 20, new LiteralText("Run"),
				button -> {
		...
		clearTextButton = addDrawableChild(
			new ButtonWidget(
				relWidth.getAsInt(), relHeight.getAsInt() + BOX_HEIGHT + 1, 20, 20, new LiteralText("X"),
				button -> {
					textBoxSelectionManager.selectAll();
					textBoxSelectionManager.delete(1);
					invalidateBoxContent();
		...
		prevButton = addDrawableChild(
			new ButtonWidget(
				relWidth.getAsInt() + BOX_WIDTH - 3 * 20 - 2 * 10, relHeight.getAsInt() - 20 - 1, 20, 20, new LiteralText("<"),
				button -> {
